...
export function LiveMatchView() {
  ...
  const { match, timerRunning, loadMatch, cleanup, persistTimer, reopenMatch } = useMatchStore()
  ...
  if (match.status === 'finished') {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4 p-4">
        ...
        <Button variant="outline" className="gap-1.5 rounded-xl" onClick={handleReopen} disabled={reopening}>
          <RotateCcw className="h-4 w-4" />
          {reopening ? 'Reabriendo…' : 'Reabrir partido'}
        </Button>
        <Link to="/">
